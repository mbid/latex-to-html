...
\begin{enumerate}
\item A \textit{locally ringed space $(X, \mathcal{O}_X)$}
...
$\mathcal{O}_{X, x}$ is the \textit{local ring of $X$ at $x$}.
We denote $\mathfrak{m}_{X, x}$ or simply $\mathfrak{m}_x$
the maximal ideal of $\mathcal{O}_{X, x}$. Moreover, the
\textit{residue field of $X$ at $x$} is the residue field
$\kappa(x) = \mathcal{O}_{X, x}/\mathfrak{m}_x$.
\item A \textit{morphism of locally ringed spaces}
...
$\mathcal{O}_X$ as the \textit{structure sheaf of $X$}.
...
We say that $f$ is an \textit{open immersion} if
...
is the \textit{open subspace of $X$ associated to $U$}.
\end{definition}
...
We say that $i$ is a \textit{closed immersion} if:
\begin{enumerate}
...
is the \textit{closed subspace of $X$ associated to
...
Let $R$ be a ring.
\begin{enumerate}
\item The \textit{structure sheaf $\mathcal{O}_{\Spec(R)}$ of the
...
the \textit{spectrum} of $R$ and denoted $\Spec(R)$.
...
An \textit{affine scheme} is a locally ringed space isomorphic
as a locally ringed space to $\Spec(R)$ for some ring $R$.
A \textit{morphism of affine schemes} is a morphism in the category
of locally ringed spaces.
\end{definition}
...
% LATEX_TO_HTML_IGNORE
\begin{history}
In \cite{EGA1} what we call a scheme was called a ``pre-schéma'' and the
name ``schéma'' was reserved for what is a separated scheme in the
...
% LATEX_TO_HTML_IGNORE
\end{history}
A \textit{scheme} is a locally ringed space with the property that
every point has an open neighbourhood which is an affine scheme.
A \textit{morphism of schemes} is a morphism of locally
...
We will say $U \subset X$ is an \textit{affine open}, or an \textit{open affine}
...
Let $X$ be a scheme.
\begin{enumerate}
\item A morphism of schemes is called an \textit{open immersion}
if it is an open immersion of locally ringed spaces (see
Definition \ref{definition-immersion-locally-ringed-spaces}).
\item An \textit{open subscheme} of $X$ is an open subspace of $X$
...
\item A morphism of schemes is called a \textit{closed immersion}
if it is a closed immersion of locally ringed spaces (see
Definition \ref{definition-closed-immersion-locally-ringed-spaces}).
\item A \textit{closed subscheme} of $X$ is a closed subspace of $X$
...
\item A morphism of schemes $f : X \to Y$ is called an \textit{immersion},
or a \textit{locally closed immersion} if it can be factored as
...
Grothendieck and Dieudonné first define closed subschemes
...
We define a \textit{locally closed subscheme of $X$} as a pair
...
we say that \textit{$Z$ is contained in $Z'$} simply if the morphism
...
Let $X$ be a scheme. We say $X$ is \textit{reduced} if every local ring
...
A \textit{scheme structure on $Z$} is given by a closed subscheme $Z'$ of
$X$ whose underlying set is equal to $Z$. We often say
``let $(Z, \mathcal{O}_Z)$ be a scheme structure on $Z$'' to
indicate this. The \textit{reduced induced scheme structure}
on $Z$ is the one constructed in Lemma \ref{lemma-reduced-closed-subscheme}.
The \textit{reduction $X_{red}$ of $X$} is the reduced induced scheme
...
This is called the \textit{functor of points of $X$}.
...
$q : \Spec(L) \to X$ are \textit{equivalent} if there exists
...
We say that $F$ is \textit{representable by a scheme}
or \textit{representable} if there exists a scheme $X$
...
In this case we also say \textit{the pair $(X, \xi)$ represents $F$}.
The element $\xi \in F(X)$
is often called the \textit{``universal family''} for reasons that will become
...
\begin{enumerate}
\item We say that $F$ \textit{satisfies the sheaf property for the
...
\item A \textit{subfunctor $H \subset F$} is a rule that associates
...
$H \subset F$ is \textit{representable by open immersions}
...
the \textit{fibre product} is a scheme $X \times_S Y$ together
...
closed subscheme of $Y$. The \textit{inverse image $f^{-1}(Z)$ of the
...
is more generally to work with schemes over a given \textit{base scheme},
often denoted $S$. We use the language: ``let $X$ be a scheme over $S$''
to mean simply that $X$ comes equipped with a morphism $X \to S$.
In diagrams we will try to picture the \textit{structure morphism}
...
Let $S$ be a scheme.
\begin{enumerate}
\item We say $X$ is a \textit{scheme over $S$} to mean that $X$
comes equipped with a morphism of schemes $X \to S$.
The morphism $X \to S$ is sometimes called the
\textit{structure morphism}.
\item If $R$ is a ring we say
$X$ is a \textit{scheme over $R$} instead of
$X$ is a scheme over $\Spec(R)$.
\item A \textit{morphism $f : X \to Y$ of schemes over $S$}
...
morphism of schemes. The \textit{base change} of $X$
is the scheme $X_{S'} = S' \times_S X$ over $S'$.
\item Let $f : X \to Y$ be a morphism of schemes over $S$. Let $S' \to S$
be a morphism of schemes. The \textit{base change} of $f$ is
...
Let $R \to R'$ be a ring map. The \textit{base change} $X_{R'}$
...
We say that $\mathcal{P}$ is \textit{preserved under arbitrary base change},
or simply that $\mathcal{P}$ is \textit{preserved under base change}
if whenever $X/S$
has $\mathcal{P}$, any base change $X_{S'}/S'$ has $\mathcal{P}$.
\item Let $\mathcal{P}$ be a property of morphisms of schemes over a base.
We say that $\mathcal{P}$ is \textit{preserved under arbitrary base change},
or simply that \textit{preserved under base change} if whenever
...
The \textit{scheme theoretic fibre $X_s$ of $f$ over $s$},
or simply the \textit{fibre of $f$ over $s$},
...
% LATEX_TO_HTML_IGNORE
\noindent
A scheme is \textit{quasi-compact} if its underlying topological space is
quasi-compact. There is a relative notion which is defined as follows.

\begin{definition}
\label{definition-quasi-compact}
A morphism of schemes is called \textit{quasi-compact}
...
\textit{universally closed} if every base change
...
\textit{satisfies the existence part of the valuative criterion}
...
dotted arrow exists. We say $f$ \textit{satisfies the uniqueness
...
% LATEX_TO_HTML_IGNORE
\noindent
A \textit{valuation ring} is a local domain maximal among the relation
...
Let $f : X \to S$ be a morphism of schemes.
\begin{enumerate}
\item We say $f$ is \textit{separated} if the diagonal morphism $\Delta_{X/S}$
is a closed immersion.
\item We say $f$ is \textit{quasi-separated} if the diagonal morphism
$\Delta_{X/S}$ is a quasi-compact morphism.
\item We say a scheme $Y$ is \textit{separated} if the morphism
$Y \to \Spec(\mathbf{Z})$ is separated.
\item We say a scheme $Y$ is \textit{quasi-separated} if the morphism
...
$\mathbf{P}^1_k$ does not even have an immersion into \textit{any} affine scheme.
\end{example}
...
A morphism of schemes is called a \textit{monomorphism} if it is
